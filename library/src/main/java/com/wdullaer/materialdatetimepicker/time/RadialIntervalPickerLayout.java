 ...
 */
public class RadialIntervalPickerLayout extends FrameLayout implements OnTouchListener{
    ...
    private BaseRadialTextsView mHourRadialTextsView;
    private RadialIntervalTextsView mMinuteRadialTextsView;
    private BaseRadialTextsView mSecondRadialTextsView;
    private BaseRadialSelectorView mHourRadialSelectorView;
    private RadialIntervalSelectorView mMinuteRadialSelectorView;
    private BaseRadialSelectorView mSecondRadialSelectorView;
    private View mGrayBox;

    private int[] mSnapPrefer30sMap;
    private int[] mSnapToIntervalMap;
...
//    }
    ...
    public RadialIntervalPickerLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        mMinuteRadialSelectorView = new RadialIntervalSelectorView(context);
        ...
        mMinuteRadialTextsView = new RadialIntervalTextsView(context);
    ...
    public void setInterval (int interval) {
        ...
        preparePrefer30sMap();
        mMinuteRadialTextsView.setInterval(interval);
        mMinuteRadialSelectorView.setInterval(interval);
    }
     ...
     * @param is24HourMode
     */
    public void initialize(Context context, TimePickerController timePickerDialog,
    ...
    private void preparePrefer30sMap() {
        ...
        mSnapToIntervalMap = new int[361];

        int degreesInterval = (360/60) * interval;
        snappedOutputDegrees = 0;
        for (int degrees = 0; degrees < 361; degrees++) {
            // Save the input-output mapping.
            snappedOutputDegrees = degrees -(degrees % degreesInterval);
            mSnapToIntervalMap[degrees] = snappedOutputDegrees;
        }
    }
    ...
    private int snapPrefer30s(int degrees) {
        ...
        if (isMinuteShowing()) {
            return mSnapToIntervalMap[degrees];
        } else {
    ...
    private boolean isMinuteShowing () {
        return mCurrentItemShowing == MINUTE_INDEX;
    }
        ...
        switch(event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
                    ...
                    mDownDegrees = getDegreesFromCoords(eventX, eventY, forceLegal || isMinuteShowing(), isInnerCircle);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mDownDegrees != -1 || isMinuteShowing()) {
                    degrees = getDegreesFromCoords(eventX, eventY, mDoingMove || isMinuteShowing(), isInnerCircle);
                    if (degrees != -1) {
